// Trading Statistics and Performance Service

import type { Trade, PerformanceMetrics } from '@/lib/types';
...
export class PerformanceService {
  ...
  private getAssetStats(trades: Trade[]): Map<string, { pnl: number; count: number }> {
    const assetStats = new Map<string, { pnl: number; count: number }>();
  ...
  getWinRatePercentage(trades: Trade[]): number {
  ...
  getConsecutiveLosses(trades: Trade[]): number {
    let maxConsecutive = 0;
    let currentConsecutive = 0;
